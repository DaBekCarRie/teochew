...
export function QuizHeader({ title, current, total, onBack }: QuizHeaderProps) {
  function handleClose() {
    Alert.alert('ออกจาก Quiz?', 'ความคืบหน้าจะไม่ถูกบันทึก', [
  ...
  return (
    <View className="h-14 px-5 flex-row items-center">
      ...
      <Text
        ...
        accessibilityLabel={`ข้อที่ ${current} จาก ${total}`}
      >
        {current}/{total}
      </Text>
